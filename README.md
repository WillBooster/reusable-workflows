...
Note: this repository is mirrored to `WillBoosterLab/reusable-workflows` with `one-way-git-sync` via the `sync` script, which maintainers run from their machines (`bun run sync`; `renovate.json` and `node_modules` are excluded). The mirror is not synced automatically on merge, so it can lag `main` — run `bun run sync` after merging changes that WillBoosterLab callers need. The script passes `-b main` explicitly: without it, a clone failure inside `one-way-git-sync` used to push to a branch literally named `undefined` while still reporting success, so confirm the mirror's `main` actually moved (`gh api repos/WillBoosterLab/reusable-workflows/git/ref/heads/main --jq .object.sha`) rather than trusting the exit code.
